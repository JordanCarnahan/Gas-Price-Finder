...
export default function FavoritesScreen() {
  ...
  const favoriteRows = useMemo(
    ...
    [favoriteStationKeys, fuelEconomy, gallonsNeeded, maxDistance, rows, selectedFuel, sortOrder, userCoords]
  );
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right", "bottom"]}>
      <View style={styles.screen}>
        ...
        <ScrollView
          ...
          {!loading && favoriteRows.length === 0 && !errorMessage ? (
            <View style={styles.emptyCard}>
              <Ionicons color="#ff9f4a" name="star-outline" size={24} />
              <Text style={styles.emptyTitle}>No favorites yet</Text>
              <Text style={styles.emptyBody}>
                Star stations on the home screen to keep them here. Current fuel filter: {fuelLabels[selectedFuel]}.
              </Text>
            </View>
          ) : null}
        </ScrollView>

        <FuelFinderTabBar activeTab="favorites" />
      </View>
    </SafeAreaView>
  );
}
